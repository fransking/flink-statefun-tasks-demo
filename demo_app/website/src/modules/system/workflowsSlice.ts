...
interface WorkflowsState {
    ...
    pipelines: Record<string, TaskOrGroup[]>;
    nestedPipelines: Record<string, TaskOrGroup[][]>;
    results: Record<string, unknown>;
    status: Record<string, string | null>;
}

interface RunWorkflowArg {
    api: string;
    id: string;
}
...
export const runWorkflow = createAsyncThunk('runWorkflow', async (data: RunWorkflowArg, thunkAPI) => {
    const {api, id} = data
...
const markTaskStatus = (task_id: string, status: string, pipeline: TaskOrGroup[]) => {
    if (!pipeline) {
        return
    }
...
const markTaskStatuses = (statuses: Record<string, { status: string }>, pipeline: TaskOrGroup[]) => {
    if (!pipeline) {
        return
    }
...
const markAllPendingTaskStatuses = (status: string, pipeline: TaskOrGroup[]) => {
    if (!pipeline) {
        return
    }
...
const workflowsSlice = createSlice({
    ...
    initialState,
    reducers: {
        onTaskEvent(state, action: PayloadAction<Record<string, unknown>>) {
            switch (action.payload.type) {
                ...
                case 'TASK_FINISHED_BATCH': {
                    ...
                    Object.keys(grouped).forEach(pipelineId => {
                        ...
                        const pipelines = [pipeline].concat(nestedPipelines)
                        const statuses = grouped[pipelineId].reduce<Record<string, { status: string }>>((acc, curr) =>(acc[curr['task_id'] as string] = curr as unknown as { status: string }, acc), {});
